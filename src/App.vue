...
<template>
  <div class="vsm-side-menu-wrapper" h-screen w-16 dark:bg-neutral-700 bg-white fixed inset-0 flex="~ col" justify-between>
    <div class="vsm-side-menu" mt-8 flex="~ col" gap-y-2>
      <div h-10 leading-14 rounded mx1 color-gray hover="dark:bg-neutral-600 bg-neutral3" text-center>
  ...
  <div class="split" pl-16>
    <div class="vsm-header-wrapper" w="[calc(100vw-4rem)]" px2 py1 h-16 bg="dark:neutral-800 gray-50" fixed left-16 top-0>
      <div data-tauri-drag-region flex="~ row" justify-between bg="dark:neutral7 white" color-neutral7 dark:color-neutral4 mx--2 px2 py1 my--1 h8>
        <div select-none>
          header
        </div>
        <div btn class="dark:i-carbon-moon i-carbon-light" @click="toggleDark()" />
      </div>
    </div>
    ...
    <div id="split-1" mt-16 h="[calc(100vh-4rem)]">
      ...
      <div h-full mt4 text-sm grid="~ cols-2 auto-rows-min gap4">
        <div>
        ...
        <div v-show="show" class="vsm-dialog-wrapper" w-screen h-screen z-99 fixed inset-0>
          <div class="vsm-dialog-mask" fixed inset-0 bg="black/40" @click="toggleShow()" />
          <div class="vsm-dialog-card" ma relative top="30%" w-sm h-auto bg-neutral7 rounded shadow="~ md" p4>
